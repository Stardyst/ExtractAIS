...
An interrupted active unit restarts; completed units are skipped. Process exit is the resource boundary: it releases DuckDB native allocator state, caches, threads and handles that closing a connection alone may retain in a long-lived Python process. Result delivery has a bounded worker-exit wait, so interpreter shutdown cannot block the parent indefinitely. After the first worker failure, no new unit starts; already active units drain and commit before the original error is raised. Global units use one worker with the global thread/memory profile. MMSI-bucket units use one spawned worker on the production HDD with a bounded thread, memory and task-specific temporary-directory profile.
...
Each full-archive bucket is built through three bounded queries with Parquet materialization between them: exact coordinate/time deduplication, same-time coordinate-conflict detection, and vessel sequencing with gap, distance and implied-speed flags. Closing the connection between queries releases blocking window state before the next sort. Intermediate files and DuckDB spill files share a deterministic task directory that is removed before a retry and after completion. The final file is sorted by MMSI and point sequence. Partition manifests retain compressed input/output bytes, output file count, rows, effective MiB/s and storage-budget observations.
